...
import { getMonthlyOverview } from "@/lib/server/metrics";
import { getServerTimeZone } from "@/lib/server/timezone";
import type { MonthlyOverviewInput } from "@/lib/types/metrics";
import {
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const payload: MonthlyOverviewInput = {
      rates: body.rates,
      filters:
        mergedFilters.customerId !== undefined || mergedFilters.year !== undefined
          ? mergedFilters
          : undefined,
      monthCount: body.monthCount ?? search.monthCount,
    ...
    logError("POST /api/metrics/monthly-overview failed", error, { requestId });
