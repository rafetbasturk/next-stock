...
import { getPaginatedOrders } from "@/lib/server/orders";
import { ordersSearchSchema } from "@/lib/types/search";
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const parsedSearch = ordersSearchSchema.safeParse(
    ...
    const paginated = await getPaginatedOrders({
      data: parsedSearch.data,
      timeZone,
    });
    return jsonOk(paginated);
  } catch (error) {
    logError("GET /api/orders/paginated failed", error, { requestId });
